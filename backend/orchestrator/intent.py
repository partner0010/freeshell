...
def analyze_intent(request: Dict[str, Any], user_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Intent 분석 (Memory 통합)
    ...
    # 파라미터 추출
    parameters = _extract_parameters(prompt, request, user_context)
...
def _extract_type_from_prompt(prompt: str) -> str:
    ...
    if any(kw in prompt for kw in ['숏폼', 'shortform', 'short-form', '15초', '30초', '60초']):
        ...
        return 'voice'
    elif any(kw in prompt for kw in ['캐릭터', 'character', '아바타']):
        return 'character'
    elif any(kw in prompt for kw in ['공간', 'spatial', '방', 'room']):
        return 'spatial'
    else:
...
def _extract_parameters(
    ...
    if 'style' not in parameters:
        ...
        if user_context:
            ...
            parameters['style'] = preference.get('style', request.get('style', 'animation'))
        else:
