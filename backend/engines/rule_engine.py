...
class RuleEngine(BaseEngine):
    ...
    def run(self, task_name: str, parameters: Dict[str, Any]) -> EngineResult:
        ...
        try:
            ...
            elif task_name == 'create_character':
                return self._create_character(parameters)
            elif task_name == 'create_space':
                return self._create_space(parameters)
            else:
                return EngineResult(
                    success=False,
    ...
    def _create_character(self, parameters: Dict[str, Any]) -> EngineResult:
        """캐릭터 생성"""
        prompt = parameters.get('prompt', '')
        style = parameters.get('style', '2d')
        
        character = {
            'name': prompt[:20] if prompt else 'Unnamed Character',
            'style': style,
            'visual': {
                'image': 'templates/default_character.png'
            }
    ...
    def _create_space(self, parameters: Dict[str, Any]) -> EngineResult:
        """공간 생성"""
        name = parameters.get('name', 'Unnamed Space')
        layout = parameters.get('layout', 'open')
