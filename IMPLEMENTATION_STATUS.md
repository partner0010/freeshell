...
#### 핵심 구현
- ✅ `orchestrator.py`: 중앙 제어, Ethics Guard 통합
- ✅ `intent.py`: 프롬프트 분석, 타입 추출
- ✅ `planner.py`: Task Plan 생성 (shortform, image, motion, voice)
- ✅ `executor.py`: AI → Rule → Fallback 순서 실행
...
- ✅ Scene JSON 처리 (`scene_processor.py`)
- ✅ FFmpeg 실제 명령 생성 (`ffmpeg_renderer.py`)
- ✅ GPU/CPU 분기 (`renderer_factory.py`)
...
- ✅ FFmpeg 기반 적용 (`motion_applier_v2.py`)
