...
class ContentType(Enum):
    ...
    VIDEO = "video"
    TEXT = "text"
    MEMORIAL = "memorial"  # 추모 콘텐츠


class PurposeType(Enum):
    """사용 목적"""
    MEMORIAL = "memorial"  # 추모
    PERSONAL_ARCHIVE = "personal_archive"  # 개인 보관
    EDUCATIONAL = "educational"  # 교육
    COMMERCIAL = "commercial"  # 상업적
    UNKNOWN = "unknown"
...
class EthicsGuard:
    ...
    PROHIBITED_KEYWORDS = [
        '복제', '모방', '사기', '속임', '위조',
        'clone', 'impersonate', 'fraud', 'fake', 'deceive'
    ]
    
    # 위험 키워드
    RISK_KEYWORDS = [
        '정치', '선거', '투표', '의사결정',
        'politics', 'election', 'vote', 'decision'
    ]
    ...
    def register_consent(self, consent: ConsentData) -> bool:
        """동의 등록"""
        # 동의 검증
    ...
    def assess_risk(
        ...
        if any(kw in prompt for kw in self.PROHIBITED_KEYWORDS):
            ...
            reasons.append("금지 키워드 감지")
            blocked = True
        
        # 2. 미성년자 관련
        if any(kw in prompt for kw in self.MINOR_KEYWORDS):
            risk_level = RiskLevel.CRITICAL
            reasons.append("미성년자 관련 콘텐츠")
            blocked = True
        ...
        if not consent:
            if purpose == PurposeType.MEMORIAL:
                ...
                reasons.append("추모 콘텐츠는 동의 필수")
                required_actions.append("법정 대리인 동의 필요")
            elif purpose == PurposeType.COMMERCIAL:
                risk_level = RiskLevel.HIGH
                reasons.append("상업적 이용은 동의 필수")
                required_actions.append("상업적 이용 동의 필요")
            else:
                risk_level = RiskLevel.MEDIUM
                warnings.append("동의가 권장됩니다")
        
        # 4. 생존 인물 확인
        if consent and consent.subject_status == "living":
            if not consent.consent_type == "self":
                risk_level = RiskLevel.HIGH
