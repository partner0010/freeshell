...
echo [1/7] .github 폴더 정리...
cd .github

REM 중복 문서 파일 삭제 (통합본으로 대체)
echo 중복 문서 파일 삭제 중...
...
if exist "DNS_STATUS_CHECK.md" del "DNS_STATUS_CHECK.md"

REM 중복 배치 파일 삭제 (통합본으로 대체)
echo 중복 배치 파일 삭제 중...
...
if exist "fix-dns.bat" del "fix-dns.bat"

cd ..
echo ✅ .github 폴더 정리 완료
echo.

echo [2/7] public 폴더 정리...
if exist "public\next.config.optimized.js" (
    echo ⚠️  public/next.config.optimized.js 삭제 중...
    del "public\next.config.optimized.js"
    echo ✅ 삭제 완료
) else (
...
echo [4/7] src/lib 중복 확인...
echo.
echo 확인 결과:
echo   - lib/scheduling vs lib/scheduler: 둘 다 사용 중 (유지)
echo   - lib/documentation vs lib/docs: 둘 다 사용 중 (유지)
echo   - lib/integrations/grip-bridge.ts: 삭제됨 (GRIP 브랜딩)
echo.
echo ✅ src/lib 폴더 확인 완료
echo.
...
echo [6/7] 임시 파일 확인...
for /r %%f in (*.log *.tmp *.bak *.old *.swp *.swo) do (
    echo 발견: %%f
    del "%%f" 2>nul
)
echo ✅ 임시 파일 정리 완료
